...
% sec((pi./2.1).*x).*csc(x).*sinh(x)

clear all

fun = @(x) sec((pi./2.1).*x).*csc(x).*sinh(x);
M = 129; % # sample points, similar results for m >> 128
...
% % optional for plot:
% d = abs(fun(0)-rfit(0)); % d for Vector fitting
% rfit = @(x) rfit(x) + d; % translation for Vector fitting


errfunAAA = @(x) abs(fun(x)-r(x));
errfunrat  = @(x) abs(fun(x)-rrat(x));
% errfunfit = @(x) abs(fun(x)-rfit(x));


AllmaxErrorAAA = [AllmaxErrorAAA, max(errfunAAA(G))];
AllmaxErrorRat = [AllmaxErrorRat, max(errfunrat(G))];
% AllmaxErrorFit = [AllmaxErrorFit, max(errfunfit(G))];
...
title_string = strcat('Analytic Function in the Unit Disk, $f(z) = \sec(\frac{\pi}{2.1}z)\cos(z)\sinh(z)$');
