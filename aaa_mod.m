function [r,pol,res,zer,z,f,w,errvec] = aaa_mod(F,Z,S,tol,mmax)
...
% 
% References:
% Y. Nakatsukasa, O. Séte and L. N. Trefethen. (2017). THE AAA ALGORITHM FOR
% RATIONAL APPROXIMATION. SIAM Journal on Scientific Computing. 40, no.3, 
% A1494-A1522. Retrieved from 
% https://arxiv.org/pdf/1612.00337.pdfhttps://arxiv.org/pdf/1612.00337.pdf
M = length(Z); % number of sample points
if nargin<4, tol = 1e-13; end % default relative tol 1e-13
if nargin<5, mmax = 100; end % default max type (99,99)
if ~isfloat(F), F = F(Z); end % convert function handle to vector
Z = S(Z(:)); F = F(:); % work with column vectors
...
r = @(zz) feval(@rhandle,zz,z,f,w,S); % AAA approximant as function handle
[pol,res,zer] = prz(r,z,f,w); % poles, residues, and zeros
[r,pol,res,zer,z,f,w] = ...
cleanup(r,pol,res,zer,z,f,w,Z,F,S); % remove Frois. doublets (optional)
...
function r = rhandle(zz,z,f,w,S) % evaluate r at zz
...
function [r,pol,res,zer,z,f,w] = cleanup(r,pol,res,zer,z,f,w,Z,F,S)
